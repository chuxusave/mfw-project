...
angular.module("myApp", ["ngTable"]).value("$noteData", simpleListData);

(function () {
    "use strict";

    angular.module("myApp").controller("demoController", demoController);
    demoController.$inject = ["NgTableParams", "$scope", "$http", "$noteData"];

    function demoController(NgTableParams, $scope, $http, $noteData) {

        var self = this;

        var simpleList = $noteData;
        ...
        function save(row, rowForm) {
            $http({
                method: "get",
                url: "/modify_note",
                params: { id: row.id, place: row.place, classify: row.classify }
            }).then(function (msg) {
                alert(msg.data);
                if (msg.data == "修改成功!") {
