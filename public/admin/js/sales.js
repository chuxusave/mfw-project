...
angular.module("myApp", ["ngTable"]).value("$salesData", simpleListData);

(function () {
    "use strict";

    angular.module("myApp").controller("demoController", demoController);
    demoController.$inject = ["NgTableParams", "$scope", "$http", "$salesData"];

    function demoController(NgTableParams, $scope, $http, $salesData) {

        var self = this;

        var simpleList = $salesData;
        ...
        function save(row, rowForm, oldId) {
            if (self.isAdding) {
                $http({
                    method: "get",
                    url: "/insert_sales",
                    params: { imgUrl: row.imgUrl, discount: row.discount, price: row.price, time: row.time, content: row.content }
                }).then(function (msg) {
                    alert(msg.data);
                    row.id = msg.data;
                ...
                $http({
                    method: "get",
                    url: "/modify_sales",
                    params: { id: row.id, imgUrl: row.imgUrl, discount: row.discount, price: row.price, time: row.time, content: row.content }
                }).then(function (msg) {
                    alert(msg.data);
                    if (msg.data == "修改成功!") {
