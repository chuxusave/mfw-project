...
angular.module("myApp", ["ngTable"]).value("$newsData", simpleListData);

(function () {
    "use strict";

    angular.module("myApp").controller("demoController", demoController);
    demoController.$inject = ["NgTableParams", "$scope", "$http", "$newsData"];

    function demoController(NgTableParams, $scope, $http, $newsData) {

        var self = this;

        var simpleList = $newsData;
        ...
        function save(row, rowForm, oldId) {
            if (self.isAdding) {
                $http({
                    method: "get",
                    url: "/insert_news",
                    params: { date: row.date, title: row.title }
                }).then(function (msg) {
                    alert(msg.data);
                    row.id = msg.data;
                ...
                $http({
                    method: "get",
                    url: "/modify_news",
                    params: { id: row.id, date: row.date, title: row.title }
                }).then(function (msg) {
                    alert(msg.data);
                    if (msg.data == "修改成功!") {
