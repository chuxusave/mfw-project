...
angular.module("myApp", ["ngTable"]).value("$bannerData", simpleListData);

(function () {
    "use strict";

    angular.module("myApp").controller("demoController", demoController);
    demoController.$inject = ["NgTableParams", "$scope", "$http", "$bannerData"];

    function demoController(NgTableParams, $scope, $http, $bannerData) {

        var self = this;

        var simpleList = $bannerData;
        ...
        function save(row, rowForm, oldId) {
            if (self.isAdding) {
                $http({
                    method: "get",
                    url: "/insert_banner",
                    params: { imgUrl: row.imgUrl, imgTxt: row.imgTxt, imgPlace: row.imgPlace, imgNum: row.imgNum, imgFrom: row.imgFrom, imgDate: row.imgDate }
                }).then(function (msg) {
                    alert(msg.data);
                    row.id = msg.data;
                ...
                $http({
                    method: "get",
                    url: "/modify_banner",
                    params: { id: row.id, imgUrl: row.imgUrl, imgTxt: row.imgTxt, imgPlace: row.imgPlace, imgNum: row.imgNum, imgFrom: row.imgFrom, imgDate: row.imgDate }
                }).then(function (msg) {
                    alert(msg.data);
                    if (msg.data == "修改成功!") {
